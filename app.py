...
def cargar_datos():
    ...
    # Reducción de tipos de datos para optimizar memoria RAM
...
# =========================================================================
# ASIGNACIÓN DE COLUMNAS REALES (Detectadas en tu archivo Parquet)
# =========================================================================
col_anio = 'refYear'       # Tu columna real de años
col_flujo = 'tradeFlow'    # Tu columna real de flujo comercial (Imp/Exp)
col_valor = 'primaryValue'  # Tu columna real de montos en USD
col_pais = 'partnerDesc'    # Tu columna real de países vecinos
...
# Filtro 1: Flujo Comercial (tradeFlow)
...
# Filtro 2: Año de Análisis (refYear)
...
with tab1:
    ...
    # Construcción del gráfico temporal usando el histórico total ('df_base_flujo')
    if col_anio in df_base_flujo.columns and col_valor in df_base_flujo.columns:
        # Agrupamos por año y flujo real
        ...
        st.warning("No se pudieron mapear las columnas requeridas para las tendencias.")

with tab2:
    st.subheader("Segmentación Estructural (Autoencoder + K-Means)")
    # Usamos tus columnas reales 'pc1', 'pc2' y 'cluster' que vimos en el listado
    x_col = 'pc1'
    y_col = 'pc2'
    cluster_col = 'cluster'

    if x_col in df_filtrado.columns and y_col in df_filtrado.columns and cluster_col in df_filtrado.columns:
        # Muestreamos datos si el set es gigante para que no colapse el navegador
        df_scatter = df_filtrado if len(df_filtrado) < 40000 else df_filtrado.sample(40000, random_state=42)
        ...
        st.warning("Las variables 'pc1', 'pc2' o 'cluster' no están disponibles para el año seleccionado.")

with tab3:
    st.subheader("Distribución Geográfica del Comercio Exterior")
    # Usamos 'partnerISO' que aparece en tu lista
    iso_col = 'partnerISO'
    ...
    if iso_col in df_filtrado.columns and col_valor in df_filtrado.columns:
        ...
        st.warning("No se encontró la columna geográfica 'partnerISO' en los datos.")
