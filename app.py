...
# =========================================================================
# BARRA LATERAL (SIDEBAR): FILTROS DINÁMICOS CON SEGURIDAD
# =========================================================================
...
try:
    ...
    if col_flujo and col_flujo in df_final.columns:
        ...
        flujo_seleccionado = st.sidebar.multiselect("Flujo Comercial", flujos_dische = list(flujos_disponibles), default=list(flujos_disponibles))
    ...
    st.sidebar.error(f"Error en los filtros: {e}")
...
# KPIs en la parte superior con Bloque de Seguridad
...
with col1:
    try:
        if col_valor and col_valor in df_filtrado.columns:
            ...
            st.warning("No se encontró la columna de montos para el KPI.")
    except Exception as e:
        st.error(f"Error en KPI Monto: {e}")
...
with col2:
    try:
        if col_pais and col_pais in df_filtrado.columns:
            ...
            st.warning("No se encontró la columna de países para el KPI.")
    except Exception as e:
        st.error(f"Error en KPI Socios: {e}")
...
with tab1:
    ...
    try:
        if col_anio and col_valor:
            ...
            st.warning("Faltan columnas de Año o Valor.")
    except Exception as e:
...
with tab2:
    ...
    try:
        ...
        st.error(f"Error en gráfico de clustering: {e}")
...
with tab3:
    ...
    try:
        ...
        if iso_col and col_valor:
            ...
            st.warning("No se encontraron variables geográficas válidas.")
    except Exception as e:
        st.error(f"Error en mapa geográfico: {e}")
